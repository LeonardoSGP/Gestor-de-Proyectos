<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('admin.eventos.index') }}" class="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 text-sm">
                ← Volver a la lista
            </a>
        </div>
    </x-slot>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="md:col-span-1 space-y-6">
                    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                        <h3 class="text-lg font-bold text-gray-900 dark:text-gray-100 mb-4 border-b dark:border-gray-700 pb-2">Información General</h3>

                        <div class="mb-4">
                            <span class="block text-xs text-gray-500 dark:text-gray-400 uppercase">Nombre</span>
                            <span class="block text-gray-800 dark:text-gray-200 font-medium">{{ $evento->nombre }}</span>
                        </div>

                        <div class="mb-4">
                            <span class="block text-xs text-gray-500 dark:text-gray-400 uppercase">Descripción</span>
                            <p class="text-sm text-gray-600 dark:text-gray-300">{{ $evento->descripcion ?? 'Sin descripción' }}</p>
                        </div>
                ...
                <div class="md:col-span-2" x-data="{ open: false }">
                    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                        ...
                        <div x-show="open" class="mb-6 bg-gray-50 dark:bg-gray-900 p-4 rounded-lg border border-indigo-100 dark:border-gray-700"
                            style="display: none;">
                            <form action="{{ route('admin.eventos.criterios.store', $evento) }}" method="POST"
                                ...
                                <div class="w-24">
                                    ...
                                    <x-text-input id="ponderacion" name="ponderacion" type="number" min="1"
                                        max="100" class="block w-full h-9 text-sm" required />
                                </div>
                                <button type="submit"
                        ...
                        @if ($evento->criterios->isEmpty())
                            <div class="text-center py-8 bg-gray-50 dark:bg-gray-900 rounded-lg border-2 border-dashed border-gray-200 dark:border-gray-700">
                                <p class="text-gray-500 dark:text-gray-400 mb-2">No hay criterios definidos.</p>
                                <p class="text-xs text-gray-400 dark:text-gray-500">Define cómo serán evaluados los proyectos.</p>
                            </div>
                        @else
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                ...
                                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                    @foreach ($evento->criterios as $criterio)
                                        <tr>
                                            <td class="px-4 py-2 text-sm text-gray-900 dark:text-gray-100">{{ $criterio->nombre }}</td>
                                            <td class="px-4 py-2 text-sm text-gray-600 dark:text-gray-300">{{ $criterio->ponderacion }}%
                                            </td>
                            ...
                            @if ($evento->criterios->sum('ponderacion') != 100)
                                <p class="text-xs text-red-500 dark:text-red-400 mt-2 font-semibold">* La suma debe ser 100% exacto.</p>
                            @endif
                        @endif
                    </div>
                </div>
